...
/******** OWNER NUMBER**********/
const ownerNumber = ["5594988088531@s.whatsapp.net","5594988088531@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oin @${num.split('@')[0]}\SEJA BEM-VINDO (A)ao GRUPO *${mdata.subject}* LEIA AS REGRAS DO GRUPO PARA NN SER REMOVIDO TMJ💪`
				...
				teks = `o poha saiu pq quis,mas nem pago a coca🤬,XAU SEU VIADO (A)🥳 @${num.split('@')[0]} Seus serviços serão enterrados profundamente²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `REMOVI MEMO SA POHA FDS VAI COM DEUS IRMAO KSKZ🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`REMOVI MEMO SA POHA FDS VAI COM DEUS IRMÃO KSKS @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `os mitos do gp ae *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
